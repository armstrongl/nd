...
func (sm *SourceManager) Remove(sourceID string) error {
	...
	// array so the original is never aliased or shifted in place — a concurrent
	// reader holding an earlier snapshot of the sources slice is never mutated.
	// Mirrors the AddLocal/AddGit snapshot-and-restore pattern.
	oldSources := sm.cfg.Sources
